...
Бэкенд-приложение на **FastAPI** для генерации и управления образовательными онлайн-курсами с помощью **LangGraph, LangChain, VSELLM** и поиска по источникам.
Поддерживает полную цепочку от загрузки данных до генерации структуры и уроков, версионирование и семантический поиск.

Подробности нового AI-контура, модели, запуск и ограничения: [AI_SYSTEM.md](AI_SYSTEM.md).
Результаты проверок: [AI_VALIDATION.md](AI_VALIDATION.md).
...
* **AI-генерация**:
  ...
  * Специализированные агенты, независимый QA и ограниченный цикл исправлений
  * Теории, тесты и задания по шаблонам **Jinja2**
* **RAG-механизмы**:

  * Загрузка PDF/DOCX/TXT файлов
  * Анализ и суммаризация контента
  * Постоянные embeddings в БД и гибридный поиск с проверкой прав доступа

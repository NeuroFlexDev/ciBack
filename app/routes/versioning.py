...
def get_module_versions(module_id: int, db: Session = Depends(get_db)):
    return db.query(ModuleVersion).filter(ModuleVersion.module_id == module_id).order_by(ModuleVersion.created_at.desc()).all()

@router.get("/versions/lesson/{lesson_id}")
def get_lesson_versions(lesson_id: int, db: Session = Depends(get_db)):
    return db.query(LessonVersion).filter(LessonVersion.lesson_id == lesson_id).order_by(LessonVersion.created_at.desc()).all()

...
def test_module_crud(client, db_session, auth_user, auth_headers):
    ...
    # update
    r = client.put(f"/api/modules/{mid}", json={"title": "Module 2", "expected_revision": 1}, headers=auth_headers)
    assert r.status_code == 200
    assert r.json()["title"] == "Module 2"

    # delete
    r = client.delete(f"/api/modules/{mid}", params={"expected_revision": 2}, headers=auth_headers)
    assert r.status_code == 200
